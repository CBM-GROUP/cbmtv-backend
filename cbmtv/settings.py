...
REST_FRAMEWORK = {
    ...
    # Only views that opt in via `throttle_scope` are throttled. The password
    # reset endpoints are unauthenticated and send email, so they are capped
    # per client IP to stop them being used to spam an inbox or brute-force a
    # six-digit code.
...
# Outgoing email. The console backend is the default, so with nothing configured
# a password-reset code is written to the server log instead of being sent.
# Production needs EMAIL_BACKEND=django.core.mail.backends.smtp.EmailBackend
# plus the SMTP credentials below.
...
DEFAULT_FROM_EMAIL = os.environ.get(
    'DEFAULT_FROM_EMAIL', 'CBM TV <no-reply@cbmgroupco.com>'
)
